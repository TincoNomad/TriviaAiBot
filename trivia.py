...
#game logic
        async def juego():
            ...
            try:
                guess = await client.wait_for('message',check=check, timeout= 1.0)
...
#begining of interaction
            ...
            await message.channel.send(''''Dentro de poco comenzará el juego, serna 5 preguntas yel tema sera uno de lo cursos de Platzi
tendrán 10 segundos para escoger una respuesta''')
...
Ops, te demoraste mucho 😄, si aun no sabes sobre de cual curso hacer el juego no hay problema, revisa 
la lista y luego vuelve a colcoar el comando $trivia''')
                    print (course)

                    if int(respuesta.content) + 1 == numero:
                        #print(respuesta.content)
                        cursoElegido = course

                    elif int(respuesta.content) + 1 != numero:
                        await message.author.send('Has escogido un opcion incorrecta, por favor intenta de nuevo 😊')
                        continue
                    opcionCurso = cursoElegido[3:]
                    counter = 0
                    get_question(opcionCurso,counter)
                    getLink(opcionCurso)
                    print('luego de escojer')
                    print(course)
                else:
...
# game action.
                ...
                while counter <= 4:
                    await juego()
                    counter += 1
                await message.channel.send('Fin del juego, gracias por participar 💃🕺')
